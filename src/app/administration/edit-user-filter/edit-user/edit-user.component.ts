...
export class EditUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.user = this.route.data.map(data => data.user).behaviorSubject(new User());
    this.groups = this.user.map(user => user.groups).behaviorSubject([]);
    this.roles = this.user.map(user => user.roles).behaviorSubject([]);
    this.possibleRoles = this.route.data.map(data => data.roles);
    this.possibleGroups = this.route.data.map(data => data.groups);
